...
function hexCenter(geometry: BoardGeometry, hexId: HexId): Point {
  ...
  return (
    <g>
      ...
      {barbarianHexes.map((hexId, i) => {
        ...
        return <Barbarian key={`ba${hexId}-${i}`} x={c.x + dx} y={c.y + dy} hex={hexId} />;
      })}
      ...
      {tradeHexes.map((th) => {
        const p = hexCenter(geometry, th.hex);
      ...
      {wagons.map((w, i) => {
        ...
        return (
          <Wagon key={`wg${w.at}-${i}`} x={p.x} y={p.y} vertex={w.at} seat={w.seat} loaded={w.cargo != null} />
        );
      })}
    </g>
  );
