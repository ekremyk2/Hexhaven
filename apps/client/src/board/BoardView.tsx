// <BoardView> — the static board as SVG (docs/11 §3), and the WebGL-unavailable fallback for the
// real 3D board (`board3d/Board3D.tsx`, T-1400+). Pure: renders from a board state + GEOMETRY, no
// store access. Pieces/robber/interaction layers slot in as children.
//
// T-1404: retired the Phase-13 faux-3D tilted-tabletop look (`board/projection.ts`, the per-hex
// skirt/inset extrusion) — the WebGL board is now the shipped "3D" experience, so this file is
// simply the flat board again (byte-identical to pre-T-1210 rendering).
...
export function BoardView({
  ...
  // viewBox from the vertex extents.
  let minX = Infinity,
    minY = Infinity,
    maxX = -Infinity,
    maxY = -Infinity;
    ...
    >
      ...
      <g stroke={COAST_SAND} strokeWidth={S * 0.34} strokeLinecap="round" opacity={0.9}>
        {geometry.coastEdges.map((eid) => {
          ...
          if (!e) return null;
          const a = { x: px(vx(e.a).x), y: px(vx(e.a).y) };
          const b = { x: px(vx(e.b).x), y: px(vx(e.b).y) };
          return <line key={`c${eid}`} x1={a.x} y1={a.y} x2={b.x} y2={b.y} />;
        })}
      </g>
      ...
      {geometry.hexes.map((h) => {
        ...
        const verts = h.vertices.map((vid) => {
          const v = vx(vid);
      ...
      {geometry.hexes.map((h) => {
        ...
        return <NumberToken key={`t${h.id}`} cx={c.x} cy={c.y} value={tile.token} dimmed={robbed} />;
      })}
      ...
      {Object.entries(board.harbors).map(([eidStr, type]) => {
        ...
        dx /= len;
        dy /= len;
        const b = { x: mid.x + dx * S * 0.5, y: mid.y + dy * S * 0.5 };
        ...
        return (
          <g key={`hb${eid}`}>
            ...
            <circle cx={b.x} cy={b.y} r={S * 0.32} fill="#efe4c6" stroke="#8a6a42" strokeWidth={2} />
            {type === 'generic' ? (
              <text
                x={b.x}
                y={b.y}
              ...
              <>
                <text x={b.x} y={b.y - S * 0.09} textAnchor="middle" dominantBaseline="central" fontSize={S * 0.24}>
                  {RESOURCE_GLYPH[type]}
                </text>
                <text
                  x={b.x}
